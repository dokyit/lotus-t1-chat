...
import { ChatMessage, ModelProvider, ModelProviderFactory } from './services/modelProvider';
...
export class ChatPanel {
    ...
    private static readonly viewType = 'lotusT1Chat';
    private readonly _panel: vscode.WebviewPanel;
    private readonly _extensionUri: vscode.Uri;
    private _disposables: vscode.Disposable[] = [];
    private _modelProvider: ModelProvider | undefined;
    ...
    public dispose() {
        ...
        while (this._disposables.length) {
            const x = this._disposables.pop();            if (x) {
                x.dispose();
            }
        }
    }
     ...
     */
    private async initializeModelProvider(context: vscode.ExtensionContext) {
        try {
            const provider = await ModelProviderFactory.createProvider(context);
